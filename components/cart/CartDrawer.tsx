...
export function CartDrawer() {
  ...
  return (
    <div
      className={`fixed inset-0 z-50 flex justify-end bg-[var(--scrim)] transition-opacity duration-200 ${
    ...
    >
      <div
        ...
        aria-modal="true"
        aria-label="Panier"
        className={`flex h-full w-full max-w-md flex-col bg-fond p-6 transition-transform duration-200 ${
          isVisible ? "translate-x-0" : "translate-x-full"
        }`}
        onClick={(event) => event.stopPropagation()}
      >
        <div className="flex items-center justify-between">
          <h2 className="font-serif text-xl text-encre">Votre panier</h2>
          <button
            type="button"
            onClick={closeCart}
            aria-label="Fermer le panier"
            className="flex h-9 w-9 items-center justify-center rounded-full bg-accent text-fond transition hover:opacity-90 active:scale-85"
          >
            ✕
          </button>
        </div>
        ...
        {items.length === 0 ? (
          <p className="mt-8 text-sm text-attenue">Votre panier est vide.</p>
        ) : (
        ...
        {items.length > 0 && (
          <div className="mt-6 border-t border-filet pt-4">
            <div className="flex items-center justify-between text-lg text-encre">
              <span>Total</span>
            ...
            <Link
              ...
              onClick={closeCart}
              className="mt-4 block w-full bg-accent px-6 py-3 text-center text-sm tracking-wide text-fond transition hover:opacity-90 active:scale-[0.93]"
            >
              Payer
            </Link>
          </div>
